  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = tier4_external_api_msgs__msg__ClassifiedDiagnostics;
    is_plain =
      (
      offsetof(DataType, diagnostics_notice) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _ClassifiedDiagnostics__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_tier4_external_api_msgs__msg__ClassifiedDiagnostics(
