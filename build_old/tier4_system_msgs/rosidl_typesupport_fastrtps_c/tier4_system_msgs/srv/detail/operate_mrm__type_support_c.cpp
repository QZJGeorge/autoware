  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = tier4_system_msgs__srv__OperateMrm_Request;
    is_plain =
      (
      offsetof(DataType, operate) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _OperateMrm_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_tier4_system_msgs__srv__OperateMrm_Request(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = tier4_system_msgs__srv__OperateMrm_Response;
    is_plain =
      (
...
static size_t _OperateMrm_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_tier4_system_msgs__srv__OperateMrm_Response(
    full_bounded, is_plain, 0);
