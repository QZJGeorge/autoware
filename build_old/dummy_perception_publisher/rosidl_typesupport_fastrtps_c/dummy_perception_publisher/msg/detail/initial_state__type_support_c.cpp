  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = dummy_perception_publisher__msg__InitialState;
    is_plain =
      (
      offsetof(DataType, accel_covariance) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _InitialState__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_dummy_perception_publisher__msg__InitialState(
