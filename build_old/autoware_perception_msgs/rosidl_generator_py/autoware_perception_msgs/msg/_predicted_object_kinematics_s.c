...
bool autoware_perception_msgs__msg__predicted_object_kinematics__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("autoware_perception_msgs.msg._predicted_object_kinematics.PredictedObjectKinematics", full_classname_dest, 83) == 0);
  }
  autoware_perception_msgs__msg__PredictedObjectKinematics * ros_message = _ros_message;
  ...
  {  // predicted_paths
    ...
    Py_ssize_t size = 100;
    autoware_perception_msgs__msg__PredictedPath * dest = ros_message->predicted_paths;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!autoware_perception_msgs__msg__predicted_path__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * autoware_perception_msgs__msg__predicted_object_kinematics__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("autoware_perception_msgs.msg._predicted_object_kinematics");
  ...
  {  // predicted_paths
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->predicted_paths[i]);
      PyObject * pyitem = autoware_perception_msgs__msg__predicted_path__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
