...
set(_exported_dependencies "PCL;Sophus;autoware_auto_mapping_msgs;cv_bridge;geometry_msgs;lanelet2_core;lanelet2_extension;lanelet2_io;pcl_conversions;rcl_interfaces;rclcpp;rclcpp_components;sensor_msgs;signal_processing;std_msgs;tf2_ros;ublox_msgs;visualization_msgs")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to yabloc_common_DEFINITIONS, yabloc_common_INCLUDE_DIRS,
# yabloc_common_LIBRARIES, and yabloc_common_LINK_FLAGS.
# Additionally collect the direct dependency names in
# yabloc_common_DEPENDENCIES as well as the recursive dependency names
# in yabloc_common_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(yabloc_common_DEPENDENCIES ${_exported_dependencies})
  set(yabloc_common_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(yabloc_common_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(yabloc_common_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(yabloc_common_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(yabloc_common_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(yabloc_common_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND yabloc_common_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
