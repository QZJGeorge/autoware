  ...
  tier4_planning_msgs__msg__RerouteAvailability__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tier4_planning_msgs__msg__RerouteAvailability * data =
      (tier4_planning_msgs__msg__RerouteAvailability *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tier4_planning_msgs__msg__RerouteAvailability__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tier4_planning_msgs__msg__RerouteAvailability__fini(&output->data[i]);
        }
        return false;
      }
    }
