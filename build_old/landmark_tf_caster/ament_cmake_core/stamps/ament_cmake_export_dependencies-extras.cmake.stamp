...
set(_exported_dependencies "autoware_auto_mapping_msgs;geometry_msgs;lanelet2_extension;rclcpp;tf2_eigen;tf2_geometry_msgs;tf2_ros;autoware_auto_mapping_msgs;geometry_msgs;lanelet2_extension;rclcpp;tf2_eigen;tf2_geometry_msgs;tf2_ros")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to landmark_tf_caster_DEFINITIONS, landmark_tf_caster_INCLUDE_DIRS,
# landmark_tf_caster_LIBRARIES, and landmark_tf_caster_LINK_FLAGS.
# Additionally collect the direct dependency names in
# landmark_tf_caster_DEPENDENCIES as well as the recursive dependency names
# in landmark_tf_caster_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(landmark_tf_caster_DEPENDENCIES ${_exported_dependencies})
  set(landmark_tf_caster_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(landmark_tf_caster_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(landmark_tf_caster_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(landmark_tf_caster_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(landmark_tf_caster_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(landmark_tf_caster_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND landmark_tf_caster_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
