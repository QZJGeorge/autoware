  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = static_centerline_optimizer__srv__PlanRoute_Request;
    is_plain =
      (
      offsetof(DataType, end_lane_id) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _PlanRoute_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_static_centerline_optimizer__srv__PlanRoute_Request(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = static_centerline_optimizer__srv__PlanRoute_Response;
    is_plain =
      (
...
static size_t _PlanRoute_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_static_centerline_optimizer__srv__PlanRoute_Response(
    full_bounded, is_plain, 0);
