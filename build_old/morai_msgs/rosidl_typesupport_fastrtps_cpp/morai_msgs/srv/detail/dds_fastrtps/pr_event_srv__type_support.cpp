  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = morai_msgs::srv::PREventSrv_Request;
    is_plain =
      (
...
static size_t _PREventSrv_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_PREventSrv_Request(full_bounded, is_plain, 0);
...
// forward declaration of message dependencies and their conversion functions
// functions for morai_msgs::msg::PREvent already declared above
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = morai_msgs::srv::PREventSrv_Response;
    is_plain =
      (
...
static size_t _PREventSrv_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_PREventSrv_Response(full_bounded, is_plain, 0);
