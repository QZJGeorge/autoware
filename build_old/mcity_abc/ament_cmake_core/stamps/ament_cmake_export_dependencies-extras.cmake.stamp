...
set(_exported_dependencies "tf2_ros;nav_msgs;std_msgs;geometry_msgs;sensor_msgs;rclcpp;rclcpp_components;autoware_adapi_v1_msgs;autoware_perception_msgs;autoware_auto_system_msgs;autoware_auto_perception_msgs;tier4_system_msgs;tier4_debug_msgs;dummy_perception_publisher")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to mcity_abc_DEFINITIONS, mcity_abc_INCLUDE_DIRS,
# mcity_abc_LIBRARIES, and mcity_abc_LINK_FLAGS.
# Additionally collect the direct dependency names in
# mcity_abc_DEPENDENCIES as well as the recursive dependency names
# in mcity_abc_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(mcity_abc_DEPENDENCIES ${_exported_dependencies})
  set(mcity_abc_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(mcity_abc_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(mcity_abc_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(mcity_abc_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(mcity_abc_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(mcity_abc_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND mcity_abc_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
