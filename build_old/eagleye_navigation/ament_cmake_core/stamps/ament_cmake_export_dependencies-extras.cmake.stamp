...
set(_exported_dependencies "rclcpp;geodesy;eagleye_coordinate;sensor_msgs;rtklib_msgs;eagleye_msgs;yaml_cpp_vendor;rclcpp;geodesy;eagleye_coordinate;sensor_msgs;rtklib_msgs;eagleye_msgs;yaml_cpp_vendor")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to eagleye_navigation_DEFINITIONS, eagleye_navigation_INCLUDE_DIRS,
# eagleye_navigation_LIBRARIES, and eagleye_navigation_LINK_FLAGS.
# Additionally collect the direct dependency names in
# eagleye_navigation_DEPENDENCIES as well as the recursive dependency names
# in eagleye_navigation_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(eagleye_navigation_DEPENDENCIES ${_exported_dependencies})
  set(eagleye_navigation_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(eagleye_navigation_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(eagleye_navigation_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(eagleye_navigation_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(eagleye_navigation_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(eagleye_navigation_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND eagleye_navigation_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
