  ...
  void * untyped_ros_message)
{
  ...
  // Field name: requested_sources
  {
    ...
    if (!rosidl_runtime_c__String__Sequence__init(&ros_message->requested_sources, size)) {
      fprintf(stderr, "failed to create array for field 'requested_sources'");
      return false;
    }
    auto array_ptr = ros_message->requested_sources.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = tier4_control_msgs__msg__IsStopped;
    is_plain =
      (
      offsetof(DataType, requested_sources) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _IsStopped__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_tier4_control_msgs__msg__IsStopped(
