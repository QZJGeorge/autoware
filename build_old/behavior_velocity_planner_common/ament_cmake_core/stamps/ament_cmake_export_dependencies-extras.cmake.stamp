...
set(_exported_dependencies "autoware_adapi_v1_msgs;autoware_auto_mapping_msgs;autoware_auto_planning_msgs;autoware_auto_tf2;autoware_perception_msgs;diagnostic_msgs;geometry_msgs;interpolation;lanelet2_extension;motion_utils;motion_velocity_smoother;nav_msgs;pcl_conversions;rclcpp;rclcpp_components;route_handler;rtc_interface;sensor_msgs;tf2;tf2_eigen;tf2_geometry_msgs;tf2_ros;tier4_api_msgs;tier4_autoware_utils;tier4_planning_msgs;tier4_v2x_msgs;vehicle_info_util;visualization_msgs")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to behavior_velocity_planner_common_DEFINITIONS, behavior_velocity_planner_common_INCLUDE_DIRS,
# behavior_velocity_planner_common_LIBRARIES, and behavior_velocity_planner_common_LINK_FLAGS.
# Additionally collect the direct dependency names in
# behavior_velocity_planner_common_DEPENDENCIES as well as the recursive dependency names
# in behavior_velocity_planner_common_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(behavior_velocity_planner_common_DEPENDENCIES ${_exported_dependencies})
  set(behavior_velocity_planner_common_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(behavior_velocity_planner_common_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(behavior_velocity_planner_common_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(behavior_velocity_planner_common_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(behavior_velocity_planner_common_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(behavior_velocity_planner_common_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND behavior_velocity_planner_common_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
