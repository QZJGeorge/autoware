...
set(_exported_dependencies "autoware_auto_control_msgs;autoware_auto_vehicle_msgs;can_msgs;diagnostic_updater;geometry_msgs;message_filters;nav_msgs;pacmod3_msgs;rclcpp;std_msgs;tier4_api_msgs;tier4_api_utils;tier4_debug_msgs;tier4_external_api_msgs;tier4_vehicle_msgs;vehicle_info_util")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to pacmod_interface_DEFINITIONS, pacmod_interface_INCLUDE_DIRS,
# pacmod_interface_LIBRARIES, and pacmod_interface_LINK_FLAGS.
# Additionally collect the direct dependency names in
# pacmod_interface_DEPENDENCIES as well as the recursive dependency names
# in pacmod_interface_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(pacmod_interface_DEPENDENCIES ${_exported_dependencies})
  set(pacmod_interface_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(pacmod_interface_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(pacmod_interface_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(pacmod_interface_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(pacmod_interface_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(pacmod_interface_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND pacmod_interface_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
