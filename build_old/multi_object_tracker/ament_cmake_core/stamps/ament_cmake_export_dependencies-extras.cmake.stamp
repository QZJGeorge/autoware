...
set(_exported_dependencies "autoware_auto_perception_msgs;kalman_filter;mussp;object_recognition_utils;rclcpp;rclcpp_components;tf2;tf2_ros;tier4_autoware_utils;tier4_perception_msgs;unique_identifier_msgs")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to multi_object_tracker_DEFINITIONS, multi_object_tracker_INCLUDE_DIRS,
# multi_object_tracker_LIBRARIES, and multi_object_tracker_LINK_FLAGS.
# Additionally collect the direct dependency names in
# multi_object_tracker_DEPENDENCIES as well as the recursive dependency names
# in multi_object_tracker_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(multi_object_tracker_DEPENDENCIES ${_exported_dependencies})
  set(multi_object_tracker_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(multi_object_tracker_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(multi_object_tracker_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(multi_object_tracker_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(multi_object_tracker_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(multi_object_tracker_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND multi_object_tracker_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
