  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = tier4_vehicle_msgs::msg::ActuationStatus;
    is_plain =
      (
      offsetof(DataType, steer_status) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _ActuationStatus__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_ActuationStatus(full_bounded, is_plain, 0);
