...
class RedisSubscriberRealVehiclePublisher(BasicROS2RedisCommunicationNode):
    def __init__(self):
    ...
    def callback_redis(self):
        ...
        results = self.redis_clent.mget(redis_key_list)
...
def main(args=None):
    ...
    node = RedisSubscriberRealVehiclePublisher()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
