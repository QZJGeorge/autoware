...
namespace test_09{
  ...
  : Node("test_09", options)
  {
    ...
    sub_route_state = this->create_subscription<RouteState>(
      "/planning/mission_planning/route_state", 10, std::bind(&Test09::route_state_callback, this, std::placeholders::_1));
  ...
  void Test09::init_localization(){
    localization_msg.pose.pose.position.x = 77612.703125;
    localization_msg.pose.pose.position.y = 86800.578125;

    localization_msg.pose.pose.orientation.x = 0.0;
    localization_msg.pose.pose.orientation.y = 0.0;
    localization_msg.pose.pose.orientation.z = -0.7036491905436928;
    localization_msg.pose.pose.orientation.w = 0.7105475470700086;
  }

  void Test09::init_route_points(){
    wp0.position.x = 77606.609375;
    wp0.position.y = 86669.734375;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = -0.7256919604474582;
    wp0.orientation.w = 0.6880197515638085;
  }

  void Test09::pub_localization(){
    localization_msg.header.stamp = this->get_clock()->now();
    localization_msg.header.frame_id = "map";
    pub_local->publish(localization_msg);
  }

  void Test09::clear_route(){
  ...
  void Test09::route_state_callback(RouteState::SharedPtr msg){
    route_state_msg = *msg;
  }
}
