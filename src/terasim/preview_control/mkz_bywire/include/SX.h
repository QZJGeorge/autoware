...
struct Obstacle_S
{
    ...
    std::vector<std::vector<Rectangle>> pathVec;
};
...
struct Object_S
{
    ...
    //predict path
    std::vector< std::vector<Obj_Point_Xd_S> > pathVec;
};
...
struct PlanningEnv_S
{
    ...
    long NPN = 0;
    std::vector<MiniPoint_S> refPathVec;
};
