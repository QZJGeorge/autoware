...
class WebSubscriberTrafficLights(Node):
    ...
    def _extract_signal(json_obj):
        ...
        """        
        tls_dict = {}
        for signal_id in WebSubscriberTrafficLights.signals_constants:
            tls_dict[signal_id] = ""
            intersection_name = WebSubscriberTrafficLights.signals_constants[signal_id]["intersection_name"]
            for intersection in json_obj["intersections"]:
                if intersection["name"] == intersection_name:
                    signal_info = intersection
                    for i in range(len(WebSubscriberTrafficLights.signals_constants[signal_id]["phase_order"])):
                        phase_id = WebSubscriberTrafficLights.signals_constants[signal_id]["phase_order"][i]
                        phase = signal_info["state"]["phases"][str(phase_id)]
                        c_index = WebSubscriberTrafficLights.state2int_web[phase["color"].lower()]
                        tls_dict[signal_id] += WebSubscriberTrafficLights.signals_constants[signal_id]["sumo_light"][i][c_index]
                    break
    ...
    def run(self):
        ...
        """  
        print("Successfully start WebSubscriberTrafficLights!") 
...
def main(args=None):
    ...
    web_subscriber_traffic_lights = WebSubscriberTrafficLights()

    rclpy.spin(web_subscriber_traffic_lights)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    web_subscriber_traffic_lights.destroy_node()
    rclpy.shutdown()
