...
#include <autoware_interface_vi_cosim.hpp>

namespace autoware_interface_vi_cosim{

  AutowareInterfaceVICosim::AutowareInterfaceVICosim(const rclcpp::NodeOptions & options)
  : Node("autoware_interface_vi_cosim", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&AutowareInterfaceVICosim::autoware_state_callback, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 1000ms, std::bind(&AutowareInterfaceVICosim::on_timer, this));
  }

  void AutowareInterfaceVICosim::on_timer(){
  ...
  void AutowareInterfaceVICosim::init_localization(){
    PoseWithCovarianceStamped localization_msg;

    localization_msg.pose.pose.position.x = 53.0238037109375;
    localization_msg.pose.pose.position.y = 234.5843505859375;

    localization_msg.pose.pose.orientation.x = 0.0;
    localization_msg.pose.pose.orientation.y = 0.0;
    localization_msg.pose.pose.orientation.z = -0.7178475136734094;
    localization_msg.pose.pose.orientation.w = 0.6962003642004968;
  ...
  void AutowareInterfaceVICosim::set_route_points(){
    Pose wp0, wp1;
  ...
  void AutowareInterfaceVICosim::set_operation_mode(uint8_t mode){
  ...
  void AutowareInterfaceVICosim::set_autoware_control(bool autoware_control){
  ...
  void AutowareInterfaceVICosim::autoware_state_callback(AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(autoware_interface_vi_cosim::AutowareInterfaceVICosim)
