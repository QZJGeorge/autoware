...
#include "autoware_interface_vi_realcar.hpp"

namespace autoware_interface_vi_realcar{

  AutowareInterfaceVIRealcar::AutowareInterfaceVIRealcar(const rclcpp::NodeOptions & options)
  : Node("autoware_interface_vi_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&AutowareInterfaceVIRealcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&AutowareInterfaceVIRealcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&AutowareInterfaceVIRealcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&AutowareInterfaceVIRealcar::on_timer, this));
  }

  void AutowareInterfaceVIRealcar::on_timer(){
  ...
  void AutowareInterfaceVIRealcar::pub_vehicle_report(){
  ...
  void AutowareInterfaceVIRealcar::set_route_points(){
    Pose wp0, wp1;

    wp0.position.x = 50.20692825317383;
    wp0.position.y = 158.47097778320312;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = -0.732081232055094;
    wp0.orientation.w = 0.6812173439312122;

    wp1.position.x = 95.49371337890625;
    wp1.position.y = 132.93043518066406;
    wp1.position.z = 0.0;

    wp1.orientation.x = 0.0;
    wp1.orientation.y = 0.0;
    wp1.orientation.z = -0.027496150056123932;
    wp1.orientation.w = 0.9996219093897908;
  ...
  void AutowareInterfaceVIRealcar::set_operation_mode(uint8_t mode){
  ...
  void AutowareInterfaceVIRealcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void AutowareInterfaceVIRealcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void AutowareInterfaceVIRealcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(autoware_interface_vi_realcar::AutowareInterfaceVIRealcar)
