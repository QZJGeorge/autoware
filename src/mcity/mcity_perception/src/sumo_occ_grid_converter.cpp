...
#include <sumo_occ_grid_converter.hpp>

namespace sumo_occ_grid_converter
{
  SumoOccGridConverter::SumoOccGridConverter(const rclcpp::NodeOptions &options)
      : Node("sumo_occ_grid_converter", options)
  {
    timer_ = rclcpp::create_timer(
        this, get_clock(), 200ms, std::bind(&SumoOccGridConverter::on_timer, this));
    ...
    sub_ego_odom = this->create_subscription<Odometry>(
      "/localization/kinematic_state", 10, std::bind(&SumoOccGridConverter::odom_callback, this, std::placeholders::_1));

    init();
  }

  void SumoOccGridConverter::init(){
  ...
  void SumoOccGridConverter::on_timer(){
  ...
  void SumoOccGridConverter::odom_callback(const Odometry::SharedPtr msg){
    saved_odom_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(sumo_occ_grid_converter::SumoOccGridConverter)
