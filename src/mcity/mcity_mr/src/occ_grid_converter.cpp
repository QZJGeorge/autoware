...
#include <occ_grid_converter.hpp>

namespace occ_grid_converter
{
  OccGridConverter::OccGridConverter(const rclcpp::NodeOptions &options)
      : Node("occ_grid_converter", options)
  {
    timer_ = rclcpp::create_timer(
        this, get_clock(), 200ms, std::bind(&OccGridConverter::on_timer, this));
    ...
    sub_ego_odom = this->create_subscription<Odometry>(
      "/localization/kinematic_state", 10, std::bind(&OccGridConverter::odom_callback, this, std::placeholders::_1));

    init();
  }

  void OccGridConverter::init(){
  ...
  void OccGridConverter::on_timer(){
  ...
  void OccGridConverter::odom_callback(const Odometry::SharedPtr msg){
    saved_odom_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(occ_grid_converter::OccGridConverter)
