...
namespace autoware_to_sumo
{
      ...
      : Node("autoware_to_sumo", options)
  {
    sub_ego_odom = this->create_subscription<Odometry>(
      "/localization/kinematic_state", 10, std::bind(&AutowareToSumo::odom_callback, this, std::placeholders::_1));
  ...
  string AutowareToSumo::get_mgrs_from_odom(Odometry msg){
...
RCLCPP_COMPONENTS_REGISTER_NODE(autoware_to_sumo::AutowareToSumo)
