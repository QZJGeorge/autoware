...
#include <gnss_to_autoware.hpp>

namespace gnss_to_autoware{
  GnssToAutoware::GnssToAutoware(const rclcpp::NodeOptions & options)
  : Node("gnss_to_autoware", options)
  {
    ...
    sub_imu = this->create_subscription<Imu>(
      "/ins/imu", 10, std::bind(&GnssToAutoware::imu_callback, this, std::placeholders::_1));
    sub_odom = this->create_subscription<Odometry>(
      "/ins/odometry", 10, std::bind(&GnssToAutoware::odom_callback, this, std::placeholders::_1));
...
RCLCPP_COMPONENTS_REGISTER_NODE(gnss_to_autoware::GnssToAutoware)
