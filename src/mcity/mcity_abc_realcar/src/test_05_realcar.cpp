...
#include "test_05_realcar.hpp"

namespace test_05_realcar{

  Test05Realcar::Test05Realcar(const rclcpp::NodeOptions & options)
  : Node("test_05_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test05Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test05Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test05Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test05Realcar::on_timer, this));
  }

  void Test05Realcar::on_timer(){
  ...
  void Test05Realcar::pub_vehicle_report(){
  ...
  void Test05Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 77616.96875;
    wp0.position.y = 86796.1484375;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = 0.7013418718199207;
    wp0.orientation.w = 0.7128250688858592;
  ...
  void Test05Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test05Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test05Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test05Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_05_realcar::Test05Realcar)
