...
#include "test_04_realcar.hpp"

namespace test_04_realcar{

  Test04Realcar::Test04Realcar(const rclcpp::NodeOptions & options)
  : Node("test_04_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test04Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test04Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test04Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test04Realcar::on_timer, this));
  }

  void Test04Realcar::on_timer(){
  ...
  void Test04Realcar::pub_vehicle_report(){
  ...
  void Test04Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 158.35610961914062;
    wp0.position.y = 315.5864562988281;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = 0.7023746262639909;
    wp0.orientation.w = 0.7118074770473538;
  ...
  void Test04Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test04Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test04Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test04Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_04_realcar::Test04Realcar)
