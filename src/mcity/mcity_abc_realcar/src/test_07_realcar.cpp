...
#include "test_07_realcar.hpp"

namespace test_07_realcar{

  Test07Realcar::Test07Realcar(const rclcpp::NodeOptions & options)
  : Node("test_07_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test07Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test07Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test07Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test07Realcar::on_timer, this));
  }

  void Test07Realcar::on_timer(){
  ...
  void Test07Realcar::pub_vehicle_report(){
  ...
  void Test07Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 77557.6953125;
    wp0.position.y = 86750.40625;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = 0.6915789093529723;
    wp0.orientation.w = 0.722300915227271;
  ...
  void Test07Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test07Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test07Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test07Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_07_realcar::Test07Realcar)
