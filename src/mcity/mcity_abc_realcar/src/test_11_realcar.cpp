...
#include "test_11_realcar.hpp"

namespace test_11_realcar{

  Test11Realcar::Test11Realcar(const rclcpp::NodeOptions & options)
  : Node("test_11_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test11Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test11Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test11Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test11Realcar::on_timer, this));
  }

  void Test11Realcar::on_timer(){
  ...
  void Test11Realcar::pub_vehicle_report(){
  ...
  void Test11Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 115.01073455810547;
    wp0.position.y = 117.63897705078125;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = 0.6732817490183907;
    wp0.orientation.w = 0.739386019910261;
  ...
  void Test11Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test11Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test11Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test11Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_11_realcar::Test11Realcar)
