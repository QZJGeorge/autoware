...
#include "test_01_realcar.hpp"

namespace test_01_realcar{

  Test01Realcar::Test01Realcar(const rclcpp::NodeOptions & options)
  : Node("test_01_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test01Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test01Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test01Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test01Realcar::on_timer, this));
  }

  void Test01Realcar::on_timer(){
  ...
  void Test01Realcar::pub_vehicle_report(){
  ...
  void Test01Realcar::set_route_points(){
    Pose wp0;
  ...
  void Test01Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test01Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test01Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test01Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_01_realcar::Test01Realcar)
