...
#include "test_06_realcar.hpp"

namespace test_06_realcar{

  Test06Realcar::Test06Realcar(const rclcpp::NodeOptions & options)
  : Node("test_06_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test06Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test06Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test06Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test06Realcar::on_timer, this));
  }

  void Test06Realcar::on_timer(){
  ...
  void Test06Realcar::pub_vehicle_report(){
  ...
  void Test06Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 77623.40625;
    wp0.position.y = 86710.078125;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = -0.09302498843456075;
    wp0.orientation.w = 0.9956637743368741;
  ...
  void Test06Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test06Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test06Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test06Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_06_realcar::Test06Realcar)
