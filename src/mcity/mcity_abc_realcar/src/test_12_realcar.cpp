...
#include "test_12_realcar.hpp"

namespace test_12_realcar{

  Test12Realcar::Test12Realcar(const rclcpp::NodeOptions & options)
  : Node("test_12_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test12Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test12Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test12Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test12Realcar::on_timer, this));
  }

  void Test12Realcar::on_timer(){
  ...
  void Test12Realcar::pub_vehicle_report(){
  ...
  void Test12Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 77608.6171875;
    wp0.position.y = 86637.8828125;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = 0.692876023558961;
    wp0.orientation.w = 0.7210567356159444;
  ...
  void Test12Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test12Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test12Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test12Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_12_realcar::Test12Realcar)
