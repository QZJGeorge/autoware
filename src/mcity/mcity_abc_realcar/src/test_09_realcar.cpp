...
#include "test_09_realcar.hpp"

namespace test_09_realcar{

  Test09Realcar::Test09Realcar(const rclcpp::NodeOptions & options)
  : Node("test_09_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test09Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test09Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test09Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test09Realcar::on_timer, this));
  }

  void Test09Realcar::on_timer(){
  ...
  void Test09Realcar::pub_vehicle_report(){
  ...
  void Test09Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 77605.203125;
    wp0.position.y = 86640.125;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = -0.7258206195386526;
    wp0.orientation.w = 0.6878840223849705;
  ...
  void Test09Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test09Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test09Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test09Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_09_realcar::Test09Realcar)
