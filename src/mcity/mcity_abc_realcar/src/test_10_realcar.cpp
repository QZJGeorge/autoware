...
#include "test_10_realcar.hpp"

namespace test_10_realcar{

  Test10Realcar::Test10Realcar(const rclcpp::NodeOptions & options)
  : Node("test_10_realcar", options)
  {
    ...
    sub_autoware_state = this->create_subscription<AutowareState>(
      "/autoware/state", 10, std::bind(&Test10Realcar::autowareStateCB, this, std::placeholders::_1));
    sub_veh_state = this->create_subscription<VehicleState>(
      "/mcity/vehicle_state", 10, std::bind(&Test10Realcar::vehStateCB, this, std::placeholders::_1));
    sub_operation_mode = this->create_subscription<OperationModeState>(
      "/system/operation_mode/state", 10, std::bind(&Test10Realcar::operationModeStateCB, this, std::placeholders::_1));
    ...
    timer_ = rclcpp::create_timer(
      this, get_clock(), 100ms, std::bind(&Test10Realcar::on_timer, this));
  }

  void Test10Realcar::on_timer(){
  ...
  void Test10Realcar::pub_vehicle_report(){
  ...
  void Test10Realcar::set_route_points(){
    Pose wp0;
    
    wp0.position.x = 77545.2578125;
    wp0.position.y = 86623.2265625;
    wp0.position.z = 0.0;

    wp0.orientation.x = 0.0;
    wp0.orientation.y = 0.0;
    wp0.orientation.z = -0.8234061353888456;
    wp0.orientation.w = 0.5674524968700078;
  ...
  void Test10Realcar::set_operation_mode(uint8_t mode){
  ...
  void Test10Realcar::autowareStateCB(const AutowareState::SharedPtr msg){
    autoware_state = msg->state;
  }

  void Test10Realcar::vehStateCB(const VehicleState::SharedPtr msg){
    veh_state_msg = *msg;
  }

  void Test10Realcar::operationModeStateCB(const OperationModeState::SharedPtr msg){
    operation_mode_state_msg = *msg;
  }
}

RCLCPP_COMPONENTS_REGISTER_NODE(test_10_realcar::Test10Realcar)
